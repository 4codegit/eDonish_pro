...
func NewAppController(a fyne.App) *AppController {
	return &AppController{
		app:    a,
		window: a.NewWindow("Edonish Pro - Электронный дневник"),
	}
}
...
func (c *AppController) Run() {
	...
	c.client, err = client.NewEdonishClient(
		...
		"https://edonish.tj/auth/v1/login", // authURL - эндпоинт авторизации
	)
	if err != nil {
...
func (c *AppController) onLoginSuccess() {
	...
	c.window.Close()
	c.window = c.app.NewWindow("Edonish Pro - Дашборд")
